...
- 기준가는 항상 KRX 전일종가를 사용한다. HTS/MTS SOR ON 화면도 기준가/전일대비/등락률 기준으로 KRX 전일종가를 사용한다.
- SOR/NXT 응답의 기준가가 화면 기준가를 덮으면 안 된다.
- SOR ON 차트는 통합 흐름 확인용이면서, SOR ON 전략/백테스트의 실제 판단 데이터로 참조하고 사용할 수 있다.
- 검색식/게이트 기준봉은 KRX 정규장 일봉을 우선한다.
- 단, KRX 기준가/게이트 기준봉을 쓴다는 말은 SOR OFF로만 매매한다는 뜻이 아니다. `_AL` 일봉/분봉/체결은 SOR ON 매매 판단에 사용한다.
